...
def zero_order_train_step(model, loss_fn, x, y, epsilon=0.01, num_samples=5):
    model.eval()
    ...
    for _ in range(num_samples):
        u = torch.randn_like(x, dtype=torch.float32).to(x.device)
        u = u / torch.norm(u)

        x_pos = (x.to(torch.float32) + epsilon * u).to(torch.long)
        x_neg = (x.to(torch.float32) - epsilon * u).to(torch.long)

        with torch.no_grad():
            outputs_pos = model(x_pos)
            logits_pos = outputs_pos.logits
            loss_pos = loss_fn(logits_pos.view(-1, logits_pos.size(-1)), y.view(-1))

            outputs_neg = model(x_neg)
            logits_neg = outputs_neg.logits
            loss_neg = loss_fn(logits_neg.view(-1, logits_neg.size(-1)), y.view(-1))

        gradient_estimate = (loss_pos - loss_neg) / (2 * epsilon) * u
        approx_gradients.append(gradient_estimate)

    avg_gradient = torch.mean(torch.stack(approx_gradients), dim=0)
    ...
    with open(log_dir, "w") as log_file:
        ...
        for epoch in range(args.epochs):
            ...
            for batch in data_loader:
                ...
                cnt_batch += 1
                loss = zero_order_train_step(model, loss_fn, x, y, epsilon=0.01, num_samples=5)
