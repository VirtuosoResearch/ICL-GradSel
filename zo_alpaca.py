...
def main(args):
    ...
    if args.compute_pretrained_outputs:
        ...
        for gradient_idx, file in enumerate(os.listdir(f"./gradients/{gradient_dir}")):
            if ("outputs" in file) or ("initial" in file): continue
            tmp_gradients = np.load(os.path.join(f"./gradients/{gradient_dir}", file))
            if len(tmp_gradients.shape) != 2: continue
            data_gradients.append(tmp_gradients)
...
# from custom.alpaca_model import AlpacaModel
# from custom.alpaca_data_module import AlpacaDataModule
# from peft import get_peft_model, LoraConfig
...
#     state_dict = {key: val.clone() for key, val in lm.model.state_dict().items() if 'absmax' not in key and 'quant' not in key}
#     pretrain_norm = compute_norm(state_dict)
#     print("Norm of the original model", pretrain_norm)
