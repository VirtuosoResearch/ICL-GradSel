...
def main(args):
    ...
    for dp in test_data:
        zt = init + "Input: " + dp["input"] + " Output: "
        zt_ids = tokenizer(zt, padding="max_length", return_tensors="pt", truncation=True, max_length = args.max_length*(args.k+1))["input_ids"]
        with torch.no_grad():
            if "gpt2" in model_name: zt_embedding = model.transformer.wte(zt_ids)
            elif "opt" in model_name: zt_embedding = model.model.decoder.embed_tokens(zt_ids)
            else: zt_embedding = model.model.embed_tokens(zt_ids)
        norm_all.append(torch.norm(zt_embedding.detach()).item())

    median_value = np.median(norm_all)
    median_idx = norm_all.index(median_value)
    print("median_idx: ",median_idx)
    print("-----------------------------")
    with sdpa_kernel(SDPBackend.MATH):

        anchor_dp = test_data[median_idx]
        ...
        #     min_ii = ii
        # if (ii+1) %10==0:
        #     print(f"min_ii: {min_ii} min_error: {min_error}")
